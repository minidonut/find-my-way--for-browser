...
class Constrainer {
  constructor(customStrategies) {
    ...
    if (customStrategies) {
      ...
      for (var i = 0; i < kCustomStrategies.length; i++) {
        ...
        assert(
          strategy.deriveConstraint && typeof strategy.deriveConstraint === 'function',
  ...
  _buildDeriveConstraints() {
    ...
    if (mustMatchKeys.length > 0) {
      lines.push(
        `derivedConstraints.__hasMustMatchValues = !!(${mustMatchKeys
          .map(key => `derivedConstraints.${key}`)
          .join(' || ')})`
      );
    }
